...
const DocSections = ( { section, sections, searchValue } ) => {
    ...
    const filteredArticles = sectionArticles?.filter( doc =>
        doc?.title?.rendered?.toLowerCase().includes( searchValue.toLowerCase() )
    ) || [];

    // const sortableArticles = filteredArticles.sort( (a, b) => a.menu_order - b.menu_order ) || [];
						...
						<div className="flex items-center w-full">
							<div className="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
								<div className="truncate flex items-center">
                                    ...
                                    <RestictionModal docId={ section.id } className="ml-4 hidden group-hover:block">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5 stroke-gray-500 hover:stroke-red-700">
                                            <path
                                                strokeWidth="2"
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
