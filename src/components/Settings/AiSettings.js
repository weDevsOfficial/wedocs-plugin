...
const AiSettings = ({ settingsData, aiSettingsData, setSettings }) => {
  ...
  const getDefaultProviderAndModel = () => {
    ...
    const defaultModel =
      providers[defaultProvider]?.selected_model || 'gpt-4o-mini';
          ...
          <div className="pt-6 pb-20 px-8 grid grid-cols-4 gap-5">
                ...
                <div className="settings-field w-full max-w-[490px] mt-1 ml-auto flex-2">
                  <div className="relative">
                    <AiProviderSelect
                      ...
                      options={Object.keys(providerConfigs).map(
                        (providerKey) => ({
                          value: providerKey,
                ...
                <div className="settings-field w-full max-w-[490px] ml-auto flex-2">
                  <div className="relative">
                    <input
                      ...
                      value={
                        aiSettings.providers[aiSettings.default_provider]
                          ?.api_key || ''
                      }
                      onChange={(e) =>
                  ...
                  <div className="relative">
                    <AiModelSelect
                      value={
                        aiSettings.providers[aiSettings.default_provider]
                          ?.selected_model ||
                        providerConfigs[aiSettings.default_provider]?.models[0]
                          ?.value
                      }
                      onChange={(value) =>
                        handleProviderChange(
                      ...
                      options={
                        providerConfigs[aiSettings.default_provider]?.models ||
                        []
                      }
                    />
                  </div>
            ...
            >
                ...
                <div className="settings-field w-full max-w-[490px] ml-auto flex-2">
                  ...
                  {!hasApiKey && isPro && (
                    <p className="text-xs text-amber-600 mt-1">
                      {__(
                        'Add an API key above to enable AI Doc Summary.',
                        'wedocs'
                      )}
                    </p>
                  )}
              ...
              <div className="settings-description w-full max-w-[490px] ml-auto mt-1">
                <p className="text-sm text-[#6B7280]">
                  {__(
                    'When enabled, readers can expand a collapsible AI summary on any doc page.',
                    'wedocs'
                  )}
                </p>
              </div>
              ...
              <div className="settings-content flex items-center justify-between mt-5">
                <div className="settings-field-heading md:min-w-[300px] flex items-center space-x-2 flex-1">
                  ...
                  <div
                    className="tooltip cursor-pointer ml-2 z-[9999]"
                    data-tip={__(
                      'Force all docs to use a specific presentation mode.',
                      'wedocs'
                    )}
                  >
                    <svg
                ...
                <div className="settings-field w-full max-w-[490px] ml-auto flex-2">
                  <div className="flex items-center gap-4">
                    {[
                      ...
                      {
                        value: 'highlights',
                        label: __('Highlights only', 'wedocs'),
                      },
                    ].map((opt) => {
                      const isSelected =
                        (aiSettings.features?.ai_summaries?.display_mode ||
                          'summary') === opt.value;
                      return (
                        <label
                          key={opt.value}
                  ...
                  <p className="text-xs text-[#6B7280] mt-1">
                    {__(
                      'Force all docs to use a specific mode if needed.',
                      'wedocs'
                    )}
                  </p>
                </div>
