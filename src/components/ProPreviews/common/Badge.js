...
const Badge = ( { classes } ) => {
  ...
  return (
    <span
      ...
      className={ `pro-badge wedocs-cursor-pointer wedocs-relative wedocs-text-white wedocs-text-[10px] wedocs-py-[3px] wedocs-px-[5px] wedocs-leading-none wedocs-ml-1.5 ${ classes ? classes : '' }` }
    >
      ...
      <UpgradeTooltip classes={ `${ showTips ? 'wedocs-block' : 'wedocs-hidden' }` } />
    </span>
  );
};
