...
const FontSettings = ( { name, classes, setSettings, settingsData } ) => {
    ...
    return (
        <div className={ `${ classes ? classes : '' } flex space-x-2.5` }>
            ...
            />
            <button className="relative cursor-pointer rounded-md border border-gray-300 bg-white py-2 px-3 text-left shadow-sm hover:bg-indigo-700 hover:text-white sm:text-sm">
                { __( 'Reset', 'wedocs' ) }
            </button>
        </div>
    );
