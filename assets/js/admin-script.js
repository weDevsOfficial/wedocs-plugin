...
jQuery( document ).ready( function () {
    jQuery( '#wedocs-beta-notice' ).on( 'click', '#try-now, #no-thanks, .notice-dismiss', ( event ) => {
        ...
        jQuery.ajax( {
            ...
            type: 'POST',
            data: {
                action: 'hide_wedocs_beta_notice'
            }
        } );

        if ( event?.target?.id === 'try-now' ) {
            window.open(' //wedocs.co/wedocs-beta/', '_blank' );
        }

        // Hide the admin notice.
        jQuery( '#wedocs-beta-notice' ).remove();
    });
})(jQuery);
